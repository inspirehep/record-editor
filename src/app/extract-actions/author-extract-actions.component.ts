    ...
    this.apiService
      ...
      .then(references => {
        if (this.replaceExisting) {
          this.jsonStoreService.setIn(['authors'], references);
        } else {
          references.forEach(reference => {
            this.jsonStoreService.addIn(['authors', 0], reference);
          });
        }

        this.toastrService.clear(infoToast.toastId);
        this.toastrService.success(`${references.length} authors extracted.`, 'Success');
      }).catch(error => {
        this.toastrService.clear(infoToast.toastId);
